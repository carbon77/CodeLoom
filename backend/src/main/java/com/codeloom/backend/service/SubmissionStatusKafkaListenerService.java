...
public class SubmissionStatusKafkaListenerService {
  private static final Logger log =
      LoggerFactory.getLogger(SubmissionStatusKafkaListenerService.class);
  private final SubmissionRepository submissions;
  private final TestCaseResultRepository results;
  private final ObjectMapper mapper;

  public SubmissionStatusKafkaListenerService(
      SubmissionRepository s, TestCaseResultRepository r, ObjectMapper m) {
    submissions = s;
    results = r;
    mapper = m;
  }
  ...
  public void listenSubmissionStatus(String message) {
    try {
      SubmissionStatusChangedEvent e =
          mapper.readValue(message, SubmissionStatusChangedEvent.class);
      ...
      if (e.payload() != null && e.payload().testCaseResults() != null) {
        ...
        results.saveAll(
            e.payload().testCaseResults().stream()
                .map(
                    r ->
                        new TestCaseResult(
                            e.submissionId(),
