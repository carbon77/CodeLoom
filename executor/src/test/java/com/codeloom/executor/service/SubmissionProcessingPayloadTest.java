...
class SubmissionProcessingPayloadTest {
    ...
    SubmissionEvent event = new SubmissionEvent(submissionId, UUID.randomUUID(), problemId, "print(input())", "python");
    ...
    void acceptedContainsOnlyPublicResults() {
        ...
        when(judge.runTestCase(any(), any())).thenAnswer(i -> {
    ...
    private SubmissionStatusPayload payload(SubmissionStatus s) {
        ArgumentCaptor<SubmissionStatusPayload> c = ArgumentCaptor.forClass(SubmissionStatusPayload.class);
