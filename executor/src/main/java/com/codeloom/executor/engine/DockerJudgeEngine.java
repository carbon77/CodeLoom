...
public class DockerJudgeEngine {
  ...
  public CompilationResult compile(SubmissionContext c) {
    ...
    try {
      files.writeFile(
          volume, c.language().getSourceFileName(), c.code().getBytes(StandardCharsets.UTF_8));
  ...
  public RunResult runTestCase(SubmissionContext c, TestCase t) {
    files.writeFile(
        volumeName(c.submissionId()), "input.txt", t.getInput().getBytes(StandardCharsets.UTF_8));
    ContainerOutcome o = runContainer(c, createContainer(c, c.language().getRunCommand()));
    return new RunResult(
        o.exitCode(), o.stdout(), message(o), o.executionTimeMs(), o.memoryUsageBytes());
  }
  ...
  private ContainerOutcome runContainer(SubmissionContext c, String id) {
    ...
    boolean oom = false;
    try {
      oom = Boolean.TRUE.equals(docker.inspectContainerCmd(id).exec().getState().getOOMKilled());
    } catch (Exception ignored) {
    }
  ...
  private String createContainer(SubmissionContext c, String command) {
    boolean compiling =
        c.language().getCompileCommand() != null
            && c.language().getCompileCommand().equals(command);
    long memory =
        compiling || c.memoryUsageLimitBytes() == null
            ? DEFAULT_MEMORY_LIMIT_BYTES
            : c.memoryUsageLimitBytes();
    HostConfig h =
    ...
    return docker
        .createContainerCmd(c.language().getImage())
        .withHostConfig(h)
  ...
  public void cleanup(UUID id) {
    var v = docker.listVolumesCmd().withFilter("name", List.of(volumeName(id))).exec().getVolumes();
