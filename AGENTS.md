...
- Flow: **backend publishes** `SubmissionEvent` to topic `submissions` (key = submissionId); **executor consumes**, runs tests, and publishes `SubmissionStatusChangedEvent` to `submission_statuses`; **backend's `SubmissionStatusKafkaListenerService` consumes `submission_statuses`** and updates the row's status + persists `TestCaseResult`s (deletes prior results for that submission first). The round-trip is complete — the row no longer stays `PENDING`.
