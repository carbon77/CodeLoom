...
public class SubmissionService {
  ...
  public void sendSubmission(SendSubmissionRequest q, Authentication a) {
    Problem p =
        problems
            .findById(q.problemId())
            .orElseThrow(() -> new ProblemNotFoundException(q.problemId()));
    if (isRegularUser(a) && !p.isPublished()) throw new ProblemNotFoundException(q.problemId());
    if (tests.countAllByProblemId(q.problemId()) == 0)
      throw new NoTestCasesException(q.problemId());
    Submission s =
        repo.save(
            new Submission(
                getUserId(a), q.problemId(), q.code(), SubmissionStatus.PENDING, q.language()));
    SubmissionEvent e =
        new SubmissionEvent(s.getId(), s.getUserId(), q.problemId(), q.code(), q.language());
